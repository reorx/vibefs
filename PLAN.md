...
```
vibefs serve [--port 8080] [--host 0.0.0.0]    # Start the web server
vibefs allow <path> [--ttl 3600]                # Authorize a file, output access URL
...
3. Outputs: `http://localhost:8080/f/a3b7c2/report.txt`
4. User clicks link → vibefs checks token in SQLite:
   ...
   - Unknown token → 404
5. Default TTL: 1 hour, configurable via `--ttl` (seconds)
